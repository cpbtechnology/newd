...
class News extends DatasourceCrawler
{
	function __construct() {
	}

	function crawlFeed($updatedSince = '', $tag = '', $datafeedId = 0) {
		$this->News =& ConnectionManager::getDataSource("news");
		$results = $this->News->search($tag);
		return $this->insertDatarows($updatedSince, $tag, "news", $datafeedId, $results);
	}
}
