...
class Blogs extends DatasourceCrawler
{
	function __construct() {
	}

	function crawlFeed($updatedSince = '', $tag = '', $datafeedId = 0) {
		$this->Blogs =& ConnectionManager::getDataSource("blogs");
		$results = $this->Blogs->search($tag);
		return $this->insertDatarows($updatedSince, $tag, "blogs", $datafeedId, $results);
	}
}
